...
class CountryStateExpandableAdapter(
    var context: Context,
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        ...
        when (row.type) {
            ExpandableCountryModel.PARENT -> {
                (holder as CountryStateParentViewHolder).countryName.text =
                    row.countryParent.country
                holder.closeImage.setOnClickListener {
                    if (row.isExpanded) {
                        ...
                        row.isExpanded = true
                        holder.upArrowImg.vi = View.VISIBLE
        ...
        when (row.type) {
            ExpandableCountryModel.PARENT -> {
                for (child in row.countryParent.states) {
                    countryStateModelList.add(
                        ++nextPosition, ExpandableCountryModel(
                            ExpandableCountryModel.CHILD, child
                        )
                    )
                }
