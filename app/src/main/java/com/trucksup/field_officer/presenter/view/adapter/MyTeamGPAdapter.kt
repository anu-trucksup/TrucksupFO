...
import com.trucksup.field_officer.databinding.ItemMyTeamGpBinding
import com.trucksup.field_officer.databinding.ItemMyTeamTsActiveBinding

class MyTeamGPAdapter(var context: Context?, var list: ArrayList<String>) :
    RecyclerView.Adapter<MyTeamGPAdapter.ViewHolder>() {

    private var controllerListener: ControllerListener? = null

    inner class ViewHolder(var binding: ItemMyTeamGpBinding) : RecyclerView.ViewHolder(binding.root)

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        var v = ItemMyTeamGpBinding.inflate(LayoutInflater.from(context), parent, false)
        return ViewHolder(v)
    }
